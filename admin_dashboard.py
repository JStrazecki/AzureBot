# admin_dashboard.py - Enhanced admin dashboard with flexible Azure Function authentication
"""
Enhanced Admin Dashboard Route Handler for SQL Assistant Bot
Features:
- Flexible Azure Function authentication (URL with key, separate key, or MSI)
...
class AzureFunctionAuthHandler:
    """Handles flexible Azure Function authentication"""
    
    @staticmethod
    def extract_auth_from_url(url: str) -> tuple[str, Optional[str]]:
        """Extract base URL and code from function URL if present"""
        parsed = urlparse(url)
        ...
        # Reconstruct base URL without code parameter
        base_url = f"{parsed.scheme}://{parsed.netloc}{parsed.path}"
        
        # Add back any other query parameters (excluding 'code')
        other_params = {k: v for k, v in query_params.items() if k != 'code'}
        if other_params:
            from urllib.parse import urlencode
            base_url += '?' + urlencode(other_params, doseq=True)
        
        return base_url, code
    
    @staticmethod
    def prepare_function_request(function_url: str, function_key: str = None) -> dict:
        """Prepare headers and URL for function request"""
        base_url, url_code = AzureFunctionAuthHandler.extract_auth_from_url(function_url)
        
        # Determine which authentication method to use
        headers = {"Content-Type": "application/json"}
        final_url = function_url  # Default to original URL
        
        if url_code:
            # URL already contains the code
            final_url = function_url
            # No need to add key to headers
        elif function_key:
            # Use separate function key in header
            headers["x-functions-key"] = function_key
            final_url = base_url
        else:
            # Try MSI authentication by adding specific headers
            headers["Authorization"] = "Bearer MSI"  # Placeholder for MSI token
            final_url = base_url
        
        return {
            "url": final_url,
            "headers": headers,
            "auth_method": "url_code" if url_code else "header_key" if function_key else "msi"
        }
...
class EnhancedAdminDashboard:
    ...
    def __init__(self, sql_translator=None, bot=None):
        ...
        self.function_auth = AzureFunctionAuthHandler()
    ...
    async def dashboard_page(self, request: Request) -> Response:
        ...
        # Analyze function authentication method
        base_url, url_code = self.function_auth.extract_auth_from_url(function_url)
        auth_method = "URL contains key" if url_code else "Separate key" if function_key else "Not configured"
        
        config = {
            "botUrl": f"https://{request.host}",
            "functionUrl": function_url,
            "functionKey": "***" + function_key[-4:] if function_key else "Not set",
            "functionAuthMethod": auth_method,
            "functionBaseUrl": base_url,
...
<body>
    <div class="dashboard">
        ...
        <!-- Environment Configuration with Function Auth Info -->
        <div class="config-section">
            ...
            <div class="config-grid">
                ...
                <div class="config-display">
                    <label>Function Auth Method:</label>
                    <div class="config-value auth-method-{config["functionAuthMethod"].replace(' ', '-').lower()}">{config["functionAuthMethod"]}</div>
                </div>
                <div class="config-display">
    ...
    async def api_test_function(self, request: Request) -> Response:
        """API endpoint for testing SQL function with flexible authentication"""
        try:
            function_url = os.environ.get("AZURE_FUNCTION_URL")
            function_key = os.environ.get("AZURE_FUNCTION_KEY")
            ...
            # Prepare request with flexible authentication
            auth_config = self.function_auth.prepare_function_request(function_url, function_key)
            
            # Log authentication method being used
            auth_details = {
                "auth_method": auth_config["auth_method"],
                "has_url_code": "code=" in function_url,
                "has_separate_key": bool(function_key),
                "url_length": len(function_url)
            }
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    # If using MSI, we need to get a token first
                    if auth_config["auth_method"] == "msi":
                        try:
                            # Try to get MSI token for Azure SQL
                            token = await self._get_msi_token(session)
                            if token:
                                auth_config["headers"]["Authorization"] = f"Bearer {token}"
                        except:
                            # MSI not available, continue without it
                            pass
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                                ...
                                return json_response({
                                    ...
                                    "data": {
                                        ...
                                        "details": {
                                            "status_code": response.status,
                                            "auth_method": auth_config["auth_method"],
                                            "response": "Function responded but returned non-JSON data",
                                            "auth_details": auth_details
                                        }
                                    },
                                    "timestamp": datetime.now().isoformat()
                            ...
                            # Authentication failed - provide detailed diagnostics
                            return json_response({
                                "status": "error",
                                "data": {
                                    "success": False,
                                    ...
                                    "details": {
                                        ...
                                        "suggestions": self._get_auth_suggestions(auth_config["auth_method"])
                                    }
                                },
                                "timestamp": datetime.now().isoformat()
                            })
   ...
   async def _get_msi_token(self, session: aiohttp.ClientSession) -> Optional[str]:
       ...
       try:
           # MSI endpoint for Azure App Service
           ...
           params = {
               ...
               "resource": "https://database.windows.net/"  # Azure SQL resource
           }
           
           async with session.get(msi_endpoint, headers=headers, params=params, timeout=aiohttp.ClientTimeout(total=5)) as response:
               if response.status == 200:
                   data = await response.json()
                   return data.get("access_token")
       except:
           # MSI not available or failed
           pass
       
       return None
   
   def _get_auth_suggestions(self, auth_method: str) -> list:
       """Get suggestions based on failed auth method"""
       suggestions = []
       
       if auth_method == "url_code":
           suggestions.append("The URL contains a code parameter, but authentication still failed")
           suggestions.append("Verify the code in the URL is correct and not expired")
           suggestions.append("Try regenerating the function key in Azure Portal")
       elif auth_method == "header_key":
           suggestions.append("Using separate function key in header, but authentication failed")
           suggestions.append("Check that AZURE_FUNCTION_KEY environment variable is correct")
           suggestions.append("Make sure you're using the function key, not the host key")
       else:
           suggestions.append("No authentication method configured")
           suggestions.append("Either add the function key to the URL (?code=YOUR_KEY)")
           suggestions.append("Or set AZURE_FUNCTION_KEY environment variable")
           suggestions.append("Or configure MSI/Contributor access for the App Service")
...
- Or just ask a natural language question about your data!"""
   ...
   def _get_enhanced_dashboard_css(self) -> str:
       ...
       * {
       ...
       .config-value.auth-method-url-contains-key {
           color: #28a745;
           font-weight: bold;
       }

       .config-value.auth-method-separate-key {
           color: #17a2b8;
           font-weight: bold;
       }

       .config-value.auth-method-not-configured {
           color: #dc3545;
           font-weight: bold;
       }
...
Sample: ${result.data.details.sample_databases ? result.data.details.sample_databases.slice(0, 3).join(', ') : 'N/A'}`;
                   ...
                   log(`✅ Using authentication method: ${result.data.details.auth_method}`, 'info');
               } else {
                   const error = result.data ? result.data.error : result.error;
                   const authDetails = result.data && result.data.details ? result.data.details.auth_details : {};
                   const suggestions = result.data && result.data.details ? result.data.details.suggestions : [];
                   
                   let errorDetails = error;
                   if (authDetails.auth_method) {
                       errorDetails += `\nAuth Method: ${authDetails.auth_method}`;
                   }
                   if (suggestions && suggestions.length > 0) {
                       errorDetails += `\n\nSuggestions:\n${suggestions.join('\n')}`;
                   }
                   
                   updateStatus('sqlFunction', 'error', errorDetails);
...
Status: ${data.status}`;
       ...
       document.addEventListener('DOMContentLoaded', function() {
           ...
           // Log function authentication configuration
           if (CONFIG.functionAuthMethod) {
               log(`🔐 Function Auth: ${CONFIG.functionAuthMethod}`, 'info');
           }
